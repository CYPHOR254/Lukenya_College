...
export class EngineeringRemarksComponent implements OnInit {
  hod = {
  ...
  ngOnInit(): void {
    this.cmsCache.getStaffMember('engineering-hod').pipe(
      catchError(() => of(null))
    ).subscribe(staff => {
      if (staff) {
        this.hod = {
          ...
          imageSrc: staff.image ? this.cmsCache.imageUrl(staff.image).width(400).auto('format').url() : 'assets/images/engineering-hod.jpg',
